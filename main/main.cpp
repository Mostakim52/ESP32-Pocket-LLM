...
const uint8_t font5x7[][5] = {
    ...
    {0x36, 0x49, 0x56, 0x20, 0x50}, {0x00, 0x05, 0x03, 0x00, 0x00},
...
static void oled_clear_region(int col, int width, uint8_t page) {
    if (col < 0) col = 0;
    if (col >= 128) return;
    if (col + width > 128) width = 128 - col;
    ...
    for (int cycle = 0; cycle < 3; cycle++) {
        for (int f = 0; f < 12; f++) {
            ...
            for (int d = 0; d < dot_count; d++) {
                ...
                if (dist == 0)      pat = BRIGHT;
                else if (dist == 1) pat = DOT;
